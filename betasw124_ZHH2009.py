# -*- coding: utf-8 -*-
"""
Created on Mon Dec 19 17:14:20 2022
...
#%%
def betasw124_ZHH2009(wavelength, S, Tc, delta=None):

    # function [betasw124, bsw, beta90sw, theta]= betasw124_ZHH2009(wavelength,S,Tc,delta)
    #
    #
    ...
    def RInw(wavelength,Tc,S):
        ...
        n0 = np.matlib.repmat(1.31405,1,301)
        n1 = np.matlib.repmat(1.779e-4,1,301)
        n2 = np.matlib.repmat(-1.05e-6,1,301)
        n3 = np.matlib.repmat(1.6e-8,1,301)
        n4 = np.matlib.repmat(-2.02e-6,1,301)
        n5 = np.matlib.repmat(15.868,1,301)
        n6 = np.matlib.repmat(0.01155,1,301)
        n7 = np.matlib.repmat(-0.00423,1,301)
        n8 = np.matlib.repmat(-4382,1,301)
        n9 = np.matlib.repmat(1.1455e6,1,301)
    ...
    def rhou_sw(Tc, S):
        ...
        a0 = np.matlib.repmat(8.24493e-1,1,301)
        a1 = np.matlib.repmat(-4.0899e-3,1,301)
        a2 = np.matlib.repmat(7.6438e-5,1,301)
        a3 = np.matlib.repmat(-8.2467e-7,1,301)
        a4 = np.matlib.repmat(5.3875e-9,1,301)
        a5 = np.matlib.repmat(-5.72466e-3,1,301)
        a6 = np.matlib.repmat(1.0227e-4,1,301)
        a7 = np.matlib.repmat(-1.6546e-6,1,301)
        a8 = np.matlib.repmat(4.8314e-4,1,301)
        b0 = np.matlib.repmat(999.842594,1,301)
        b1 = np.matlib.repmat(6.793952e-2,1,301)
        b2 = np.matlib.repmat(-9.09529e-3,1,301)
        b3 = np.matlib.repmat(1.001685e-4,1,301)
        b4 = np.matlib.repmat(-1.120083e-6,1,301)
        b5 = np.matlib.repmat(6.536332e-9,1,301)
    ...
    S = np.matlib.repmat(S,301,1).T
    Tc = np.matlib.repmat(Tc,301,1).T
    ...
    wavelength = np.matlib.repmat(wavelength, np.size(S,0), 1) # a row variable
    rad = theta*math.pi/180 # angle in radian as a colum variable
    
    # nsw: absolute refractive index of seawater
    # dnds: partial derivative of seawater refractive index w.r.t. salinity
    nsw = RInw(wavelength,Tc,S)[0]
    dnds = RInw(wavelength,Tc,S)[1]
    ...
    beta_df = math.pi*math.pi/2*((wavelength*1e-9)**(-4))*Kbz*Tk*IsoComp*DFRi**2*(6+6*delta)/(6-7*delta)
    
    # volume scattering at 90 degree due to the concentration fluctuation
    flu_con = S*M0*dnds**2/density_sw/(-dlnawds)/Na
    beta_cf = 2*math.pi*math.pi*((wavelength*1e-9)**(-4))*nsw**2*(flu_con)*(6+6*delta)/(6-7*delta)
    ...
    rad124 = np.argwhere(np.rad2deg(rad)>=124)[0]
    
    
    betasw124 = np.zeros((np.size(beta90sw,0),np.size(beta90sw,1)))*np.nan
    #for i=1:length(beta90sw)
    #    betasw124(i,:) = beta90sw(i) * (  1+((cos(rad(rad124))).^2).*(1-delta)./(1+delta)  );
    #end
    betasw124 = beta90sw*(1+((math.cos(rad[rad124]))**2)*(1-delta)/(1+delta))
